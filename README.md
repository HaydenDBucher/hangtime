...
The initial customer is an OSU-area student coordinating a group that intends to go out tonight but has not agreed on a first destination. The current alternative is a fragmented workflow across group chat, social feeds, maps, venue pages, event listings, and rideshare apps. The cost is delay, decision fatigue, unnecessary travel or cover charges, missed offers, and sometimes an abandoned plan.

Students use the decision tool free. The proposed payer is a campus-area venue or promoter responsible for demand generation. Hangtime tests whether that payer values measurable group commitments and offer redemptions more than generic advertising impressions. Pricing and willingness to pay remain assumptions until a venue budget owner makes a real commitment.

## Release verification

Every deployment runs `npm ci`, `npm run check`, and `npm run build` before GitHub Pages publishes. The automated check protects the core hypothesis, honest-evidence language, economics, limitations, instrumentation, modeled-data disclosure, plan persistence, and destination-lock flow. Automated checks do not substitute for real-user observation, visual review, assistive-technology testing, or production security review.
